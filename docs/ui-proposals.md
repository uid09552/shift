...
| Planning | Scheduler (`/scheduler`) — runs, proposals, hand edits, **Verify Plan**, **Fix Plan**, *Take as Plan* |
...
| 3 | [Coverage forecast](#3-coverage-forecast-before-planning) | "Can this period even be staffed?" | Nothing new — the `preparePlan` payload has it all | S |
| 4 | [Fairness dashboard](#4-fairness-dashboard) | "Who keeps getting the weekends?" | One analysis endpoint | S |
| 5 | [Audit log page](#5-audit-log-page) | "Who changed this, and when?" | Nothing new — `listAuditLogs` exists | S |
| 6 | [Compliance view over the confirmed roster](#6-compliance-over-the-confirmed-roster) | "Does the roster we are actually working break any rule?" | Validate a date range, not just a proposal | M |
| 7 | [Absence requests and approval](#7-absence-requests-and-approval) | "Can I have the 12th off?" | Status + approver on `unavailabilities`, entitlements | L |
| 8 | [Rotation patterns](#8-rotation-patterns) | "Put Team B on the usual six-week rhythm" | Pattern storage; writes existing shift assignments | M |
| 9 | [Publish and notify](#9-publish-and-notify) | "Who needs to be told their shift moved?" | Plan versions + a notification channel | L |
| 10 | [Shift swaps and open shifts](#10-shift-swaps-and-open-shifts) | "Will anyone take my Saturday?" | An offers table and a workflow | L |
| 11 | [Qualification matrix and expiry](#11-qualification-matrix-and-expiry) | "Whose certificate runs out in March?" | Matrix is free; expiry needs a column | S → M |
| 12 | [Self-service roster feed](#12-self-service-roster-feed) | "My shifts, in my phone's calendar" | A signed ICS endpoint | M |
...
### 3. Coverage forecast (before planning)

A day × shift grid for a period you are about to plan: **required** (each shift's
`min_employees` for that weekday, summed over the workstations that run it)
against **available** (employees who may work that shift and are not away). Red
where demand exceeds supply.

This turns the most common support question — *"why is the plan thin / why is it
infeasible?"* — into something visible **before** the solve rather than after it.
Every input is in the `preparePlan` payload the optimizer is handed, so the page
can be built on the existing endpoint with no backend change.

Pairs naturally with the scheduler: show it above the *Calculate* button.
...
### 6. Compliance over the confirmed roster
...
### 7. Absence requests and approval
...
### 10. Shift swaps and open shifts
...
### 11. Qualification matrix and expiry
...
1. **Hours account**, **scenario comparison**, **coverage forecast** — three
   pages over data that already exists, each answering a question the product is
   currently silent on.
