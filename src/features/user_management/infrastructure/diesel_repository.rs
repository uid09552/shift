...
use crate::features::user_management::domain::{
...
use crate::models::{
    NewCapability, NewEmployee, NewShift, NewUnavailability, NewWorkstation,
...
#[async_trait]
impl crate::features::user_management::domain::WorkstationRepository for DieselWorkstationRepository {
    async fn create_workstation(&self, name: &str, available: bool, active_shift_id: Option<Uuid>) -> Result<crate::features::user_management::domain::Workstation, Box<dyn std::error::Error + Send + Sync>> {
        let name = name.to_string();
        let pool: Arc<DbPool> = Arc::clone(&self.pool);
        task::spawn_blocking(move || {
            let mut conn = pool.get().map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
    ...
    async fn get_workstation(&self, id: Uuid) -> Result<Option<crate::features::user_management::domain::Workstation>, Box<dyn std::error::Error + Send + Sync>> {
        let pool: Arc<DbPool> = Arc::clone(&self.pool);
        task::spawn_blocking(move || {
            let mut conn = pool.get().map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
    ...
    async fn list_workstations(&self) -> Result<Vec<crate::features::user_management::domain::Workstation>, Box<dyn std::error::Error + Send + Sync>> {
        let pool: Arc<DbPool> = Arc::clone(&self.pool);
        task::spawn_blocking(move || {
            let mut conn = pool.get().map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send + Sync>)?;
